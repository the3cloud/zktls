...
contract ZkTLSGateway {
    event RequestTLSCallBegin(bytes32 indexed prover, string url, bytes encrypted_key, uint64 max_cycle_num);
    ...
    function requestTLSCall(string calldata url, bytes calldata encrypted_key, bytes[] calldata data) public {
        emit RequestTLSCallBegin(0x0, url, encrypted_key, 500000);
